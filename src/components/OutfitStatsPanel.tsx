...
import { colorNameToHex } from "../lib/color-lexicon";
...
function ColorColumn({ title, items }: { title: string; items: OutfitTagStat[] }) {
  if (items.length === 0) {
    return (
      <div className="min-w-0 flex flex-col gap-2.5">
        <div className="text-center text-base font-bold text-stone-800">{title}</div>
  ...
  return (
    <div className="min-w-0 flex flex-col gap-2.5">
      ...
      <div className="stats-list overflow-hidden rounded-2xl">
        {items.map((item, i) => (
          <div key={item.name}>
            <ColorStatRow item={item} rank={i + 1} featured={i === 0} />
          </div>
        ))}
      </div>
    </div>
...
function ColorStatRow({
  ...
  const fill = item.hex ?? colorNameToHex(item.name);
  ...
  return (
    ...
    >
      ...
      <div
        className="h-7 w-7 shrink-0 rounded-lg border border-stone-200/80 shadow-inner"
